...
@section('content')
    ...
    <div class="col-md-8 col-md-offset-2 text-justify">
        ...
        @if($slugchapitre == $first->chapitre_slug OR !empty($prev_quizz))
            ...
            <a href="{{route('voir.cours', [$cours->domaine->slug, $cours->cours_slug])}}" class="text-left">Revenir
                vers
                le
                cours</a><br>
        ...
        @if($chap->url_video != null)
            <div class="panel-header">
                <iframe width="715" height="415" style="margin:10px;" src="{{$chap->url_video}}" frameborder="0"
        ...
        <p class="text-center">
            @foreach($prev as $p)
                <a href="{{route('voir.chapitre', [$cours->domaine->slug, $cours->cours_slug, $p->chapitre_slug])}}"
                   class="btn btn-primary float-left">{!! $p->chapitre_titre !!}</a>
            @endforeach

            @foreach($next as $n)
                <a href="{{route('voir.chapitre', [$cours->domaine->slug, $cours->cours_slug, $n->chapitre_slug])}}"
                   class="btn btn-primary float-right">{!! $n->chapitre_titre !!}</a>
            @endforeach
        </p>
        ...
        @elseif($quizz_user->isEmpty() OR $prev_quizz->isEmpty())
            ...
            <p class="text-center alert alert-danger">
                ...
                Validez le quizz du chapitre précédent !
            </p>
